...
- [x] Integrate discovery counts, commit progress, warnings, retry, and import reports with the shared application. — the offer shows discovered conversations and unavailable attachments; accepted bundles run the existing progress, warnings and report flows; the extension page shows transfer state and the final outcome. Since 2026-09-12: a storage fault during an accepted bundle's import pauses the run with its cause, reports that outcome to the extension, and "Resume selected import" finishes it from the staged bytes without a second transfer; the saved import report shows the extension provenance (extractor, source, discovery counts, page URL) in Import details ([extension-import.md](../validation/extension-import.md), 18 panel checks per engine).
