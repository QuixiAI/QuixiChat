...
- [x] Feed extracted PDF and optional OCR text through the same pipeline as messages. Extend onboarding/settings and document progress without delaying initial FTS availability. — Extracted pages flow through the same claim pipeline (document-page storage test: two identical pages share one vector and resolve by page); settings expose the §72/§73 panel; onboarding step 5 (product §94) enrols through the same controller ([onboarding qualification](../validation/onboarding.md)); lexical indexing never waits on enrolment. OCR is deferred (plan 15).
- [ ] Integrate WebGPU from plan 19 when ready and validate automatic WASM fallback. Keep a modest float-vector baseline for correctness before compressed scale work. — WebGPU is attempted first and used in WebKit; the CPU route serves Chromium headless; float32 is the only representation, and the float baseline's retrieval quality with the production chunker equals the plan 16 reference on its corpus (Recall@5 0.842593, Recall@10 0.898148, MRR 0.861111; [production-chunker.json](../../perf/retrieval/production-chunker.json)). Since 2026-09-12 the in-application device-loss fallback is observed: the embedding service exposes a diagnostics fault (`injectFault("gpu-device-loss")`, which destroys the owned GPU device so the real `device.lost` path runs), the scheduler falls back to the CPU encoder on the next dispatch, statistics carry the live route and the semantic panel reports "WASM SIMD · CPU" in place of WebGPU; the semantic proof asserts this in WebKit (where WebGPU is used) and the refusal on Chromium's CPU route ([semantic-search.md](../validation/semantic-search.md), 14 checks per engine since the plan 23 inference self-test, saved report and rebuild verification joined the proof). Open: a hardware-adapter Chromium run (Chromium here has no GPU adapter; the user's hardware run).
