...
- [ ] Validate web hosting and Docker headers/origin behavior, desktop installation and native integrations, release artifact integrity, signing/update configuration, and upgrade persistence. Keep unsupported WebViews explicitly provisional. — Web hosting/Docker is done ([web-hosting.md](../validation/web-hosting.md), 2026-09-13): the built image's isolation headers on every asset, WASM media types, SPA fallback versus asset 404s, the callback route's policy and log exclusion, and cross-origin isolation plus the storage backend in both engines; it found and fixed a missing-model 200 fallback and onboarding's unverified "model provided" claim. Desktop installation, native integrations, artifact integrity, signing/update and upgrade persistence remain.
