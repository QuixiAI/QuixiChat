...
- [ ] Run storage/import stress workloads at 100k threads, 1M+ messages, and 10–50 GB attachments. Measure startup, migration, quota failure, concurrent tabs, interrupted writes, export, and restore without archive-wide materialization.
- [ ] Run PDF cases from 1 to 1000+ pages, including malformed, encrypted, multi-column, scanned, and image-heavy inputs; report memory, extraction speed, time to FTS, cancellation, and resume.
...
- [ ] Validate web hosting and Docker headers/origin behavior, desktop installation and native integrations, release artifact integrity, signing/update configuration, and upgrade persistence. Keep unsupported WebViews explicitly provisional.
