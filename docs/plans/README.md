...
Completed plans: **02, 03, 08, 14, 16, 17, 18, 20, 22 and 23** (canonical contracts and storage,
the shared chat and library UI, document extraction/search, embedding reference, scalar
encoder, SIMD and bounded embedding scheduler, compressed large-index retrieval, diagnostics and recovery). Plans **01, 04, 05, 06, 07, 09, 10, 11, 12, 13, 19 and 21** have work underway
for platform qualification, imports, host adapters, providers, lexical search,
archives, provider switching, browser-extension import, compare/critique/migration, WebGPU release qualification and semantic search. One other in-scope plan remains planned; OCR and the two Cloud plans are deferred. Detailed evidence and
...
Latest completed iteration (2026-09-12): **critique** (plan 12, ADR 0042).
...
Next concrete slice: **plan 12 reviewed migration** — from the bulk
portability analysis, a reviewed routing change for a selected set of
conversations: per-conversation switch inspection and compaction choices,
`SetRoutingProfile` committed per conversation with a `Migration` event,
bounded, cancellable, with per-conversation blocked/failed outcomes retained
(ADR 0042); that closes plan 12. Then plan 12 (compare, critique, bulk
