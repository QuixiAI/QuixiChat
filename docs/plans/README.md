...
embedding scheduler). Plans **01, 04, 05, 06, 07, 08, 09, 10, 13, 19, 21 and 23** have work underway
for platform qualification, imports, host adapters, providers, lexical search,
shared UI, archives, provider switching, WebGPU release qualification, semantic search and diagnostics. Four other in-scope plans remain planned; OCR and the two Cloud plans are deferred. Detailed evidence and
...
Latest completed iteration (2026-09-11): **semantic indexing and hybrid search**
...
Next concrete slice: **plan 11 browser-extension import**, the next planned
plan whose prerequisites (04 importer framework, 05 host capabilities) are in
place; it is also a prerequisite of plan 13's onboarding flow (step 3 offers
the extension), so it comes before the onboarding step 5 prompt that closes
plan 21's open item. Do plan 11's research first: current provider
bundle/DOM formats, extension manifest/permission requirements and the
bounded transfer protocol into the importer, recorded with dated sources.
Then measure retrieval relevance of the production 256-token structural
chunker on the plan 16 corpus (`perf/retrieval`) to record
production-chunking parity before plan 22's compressed-retrieval design
review. Live provider behavior and summary fidelity remain separate authorized
