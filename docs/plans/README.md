...
Latest completed iteration (2026-09-12): **production-chunking parity
...
Next concrete slice: **plan 22 research and design review** (compressed
vector retrieval): read its product references and the plan 16/21 evidence,
then benchmark candidate representations (int8, sign-bit, product-quantized)
and candidate-generation designs on the frozen corpus with the production
chunker before implementing; record the decision in `docs/decisions/`. Then
plan 12 (compare, critique, bulk migration), whose prerequisites (08, 10) are
in place. Plan 13's themes and accessibility audit, plan 11's real
