...
Latest completed iteration (2026-09-12): **browser-extension import** (plan 11,
...
Next concrete slice: **plan 13 onboarding flow** (product §94 steps 1–5:
local-storage disclosure, capability check from actual host/storage state,
bring history including the extension, providers now or later, and the
semantic step 5 prompt that closes plan 21's open item) in both engines. Then
measure retrieval relevance of the production 256-token structural chunker on
the plan 16 corpus (`perf/retrieval`) before plan 22's compressed-retrieval
design review. Plan 11's remaining items need the user (a real chatgpt.com
run) or later hosts (desktop pairing); do not contact real providers. Live provider behavior and summary fidelity remain separate authorized
