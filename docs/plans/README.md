...
Latest completed iteration (2026-09-12): **compare mode** (plan 12, ADR
...
Next concrete slice: **plan 12 critique** — a Generation whose parameters
and `Critique` event name the reviewed answer, its prompt built from that
answer's parts through the request path, its output a sibling branch of the
reviewed answer's parent (ADR 0042); then the reviewed migration on top of
the bulk analysis. Then plan 12 (compare, critique, bulk
