...
Latest completed iteration (2026-09-12): **inference self-test** (plan 23,
...
Next concrete slice: **plan 23 exportable diagnostics report** — the storage
report and the inference self-test written as one JSON file through the host
save flow, with the metadata-only policy asserted on the saved bytes in both
engines; then the remaining Doctor audits (blob hashes, branch invariants, import provenance,
