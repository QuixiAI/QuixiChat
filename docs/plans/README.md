...
Latest completed iteration (2026-09-12): **Doctor audit** (plan 23). A
...
Next concrete slice: **plan 23 blob-hash audit** — verify every stored
file's bytes against its digest as a bounded, cancellable scan (reusing the
blob inventory's file iterator and the shared scan controller), then the
fault fixtures for every repair and the reviewed-cleanup criterion. Then plan 12 (compare, critique, bulk
