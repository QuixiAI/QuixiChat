...
Completed plans: **01, 02, 03, 08, 09, 12, 14, 16, 17, 18, 20, 22 and 23** (universal storage proof, canonical contracts and storage,
the shared chat and library UI, archives and open export, compare/critique/bulk migration, document extraction/search, embedding reference, scalar
encoder, SIMD and bounded embedding scheduler, compressed large-index retrieval, diagnostics and recovery). Plans **04, 05, 06, 07, 10, 11, 13, 19 and 21** have work underway
for platform qualification, imports, host adapters, providers, lexical search,
provider switching, browser-extension import, WebGPU release qualification and semantic search. One other in-scope plan remains planned; OCR and the two Cloud plans are deferred. Detailed evidence and
...
Latest completed iteration (2026-09-13): **one transaction per clean-copy
step** (plan 09/24). Each export step autocommitted 128 row inserts, a
journal write and sync per row; a step now commits once. On the
30,000-message archive-scale proof in Chromium the portable export fell
from 430 s to 58 s, restore validation from 178 s to 86 s and the open
export from 399 s to 68 s; the WebKit half of that proof is rerun once the
eighth 1M run (still exporting with the earlier bundle) releases the
shared WebKit storage. Also added `HUMAN_DO.md`, the list of every gate
that needs the user. Completed plans remain **13 of 23**.

Previous completed iteration (2026-09-13): **1M-message restore validated;
plan 24's stress task ticked**. The validator's per-node graph walk (hours
at a million messages) is now set-based Kahn batches with recursive
ancestry queries; the restore probe over the kept 4.17 GB container
validated 2,300,000 records in 71,596 bounded steps and 89 minutes, with
topology at 55 s. The full-run table is recorded from the seventh attempt,
the sixth and the probe; an eighth attempt with every fix in one bundle
runs for a single end-to-end record. The harness now also removes stale
stress archives from WebKit's shared origin storage (37 GB had
accumulated). Completed plans remain **13 of 23**.

Previous completed iteration (2026-09-13): **restore validation at scale,
first pass** (plans 24 and 09). The seventh 1M run completed its export
(9,338 s) and then sat in restore validation for over three hours with no
...
interrupted transactions, migration failure, multi-tab contention, and the
explicit Safari and WebKitGTK records) is ticked against the retained
proofs, and the plan states its restricted support: the universal path is
proven on web, Docker web and macOS desktop, no-go on Linux WebKitGTK
2.50.6, unverified on installed Safari and Windows, which remain plan 24
host gates. Completed plans: **13 of 23**.

Previous completed iteration (2026-09-13): **plan 07 gate correction**. Its
dependent-capability task listed the portability filter, document
extraction and semantic retrieval as gates; all three have landed, so the
task is ticked and the status line names what actually remains (the
relevance study and per-host scale measurements). Plan 10's compaction
task stays open only for real-model summary fidelity, a user gate.

Previous completed iteration (2026-09-13): **batched streaming checkpoints**
(plan 06). Text deltas now ride in the next checkpoint's commit or a
bounded flush (4 KiB, 16 mutations or 100 ms idle), halving commits per
streamed record (1,002 for 1,000 records, was 2,002) with raw retention,
byte bounds and the committed prefix on stop unchanged; 98 providers tests
plus the providers browser proof (which caught the need for the idle flush
on a paused stream) and the shared-app proof pass. Plan 06's only remaining gate is
live-provider qualification. The seventh 1M run finished its export in
9,338 s and is validating the restore through the stepped import.
Completed plans remain **12 of 23**.

Previous completed iteration (2026-09-13): **checkpoint density baseline**
(plan 06). A providers test now measures the streaming persistence
design: two commits per streamed record (a verified raw checkpoint and a
text mutation), 2,002 for 1,000 records, up to 200,000 per attempt at the
record ceiling; the batching design (carry the pending delta in the next
raw checkpoint's commit, flush on terminal and on stop) is recorded on the
task for the next slice. The providers suite's typecheck had been broken by
a stale fake host; it passes again (96 tests). Completed plans remain
**12 of 23**.

Previous completed iteration (2026-09-13): **release page** (plan 24 task 8).
[docs/release.md](../release.md) publishes host support, measured limits,
known issues, migration and rollback, and the release checklist from the
retained evidence; the task is ticked, with the 1M numbers to refresh when
the seventh run completes. Completed plans remain **12 of 23**.

Previous completed iteration (2026-09-13): **batched clean copy** (plan 09/24).
...
Previous completed iteration (2026-09-13): **stepped restore import and
per-table candidate integrity** (plans 24 and 09). The sixth 1M-message
run completed its export (3,980 MB in 10,214 s over 62,679 steps, digest
verified on disk) and failed at the restore's first validation step, which
copied the 4.7 GB received database into the candidate pool in one
advance; the candidate's whole-file `integrity_check` would have followed.
`BoundedFileCopier` (the export snapshot copier generalized to any byte
source) now steps the import by `maxBytes`, the integrity check runs one
table per step, and app and harness archive advances carry the 600 s
deadline. Next: rerun the 1M run; then cut the export's per-row statement
cost (one `LIMIT 1` query per copied row over 3.5 million rows). Completed
plans remain **12 of 23**.

Previous completed iteration (2026-09-13): **desktop release bundle proof**
(plan 24 task 7, artifact part). `tests/hosts/desktop-bundle-proof.mjs`
reads the produced macOS app and disk image: identity and version from the
bundle, `hdiutil` checksum verification, SHA-256 inventory of the image,
executable and embedded frontend, and the signature state as found. It
found the bundle only linker-signed (deep verification failed); the config
...
built Docker image on loopback and checks isolation headers on every
asset, WASM media types, SPA fallback versus asset 404s, the callback
route's policy and log exclusion, and in both engines cross-origin
isolation and the storage backend (41 HTTP checks, 7 per engine). It found
two defects: nginx answered a missing model with the application document
(now a 404 through a `/models/` location), and onboarding claimed a
host-provided model whenever one was configured (it now probes the URL;
unit tests and the onboarding browser proof pass). The 1M-message run is
still in its export phase. Completed plans remain **12 of 23**.

Previous completed iteration (2026-09-13): **PDF scale evidence consolidated**
(plan 24 task 6). [pdf-scale.md](../validation/pdf-scale.md) maps the
requested 1–1,000+ page, malformed, encrypted, multi-column, scanned and
image-heavy cases to plan 14's retained browser evidence and the production
performance harness (memory, speed, time to FTS, cancellation, resume), with
the declared limits and the unobserved WebKit/native memory named; the task
is ticked. The sixth 1M-message run passed integrity (66 s) and cold reopen
and is in its export phase; its result is recorded next. Completed plans
remain **12 of 23**.

Previous completed iteration (2026-09-13): **stress harness deadline defect**
(plan 24). The 1M-message run's fourth and fifth attempts failed at the
diagnostics report at exactly 60 s although it was requested with the 600 s
deadline; a new probe tool over the kept browser profile
(`probe-archive.mjs`, logging the stack of every long timer) showed the
harness's own fault-injecting `storage.request` wrapper dropping the
per-request options. The wrapper now forwards them and the probe completed
the report on the same 4.9 GB archive in 67 s. The run is relaunched; its
result is recorded next. Completed plans remain **12 of 23**.

Previous completed iteration (2026-09-13): **stepped export snapshot** (plans
24 and 09, [ADR 0010](../decisions/0010-portable-archives.md) amendment).
The 1M-message run's third attempt confirmed the two earlier fixes
(integrity 58 s over 4.7 GB; library first page 9 ms, twenty pages 30 ms;
cold reopen 0.36 s; follower tab 79 ms) and failed at the export's
`snapshot` phase, which copied the whole 4.7 GB database inside one
advance step. `SnapshotCopier` now copies at most `maxBytes` per step through the VFS
into a pool file associated with its name (the pool's chunked import holds
an unassociated handle, which another file open between steps could take),
holds the read transaction across steps and unlinks its output on release;
a Node test drives it step by step against the independent export and
both release orders; the archives suite, its browser proof and the stress smoke
pass. Next: rerun the 1M run and record it. Completed plans remain
**12 of 23**.

Previous completed iteration (2026-09-13): **materialized library activity**
(plans 24 and 08, [ADR 0043](../decisions/0043-materialized-library-activity.md)).
The 1M-message run's second attempt passed integrity and failed at library
paging (25.4 s for the first page at 100,000 conversations) because the
view recomputed and sorted every thread's activity once per item. Schema
13 adds one trigger-maintained activity row per thread with an index in
library order; a page is one index walk joined to its own thread states,
and retained archives at older schemas keep the previous query. The
canonical suite compares both readings before and after the upgrade and
after every kind of change (58 tests); the schema-13 assertions in the
browser runners are updated. At the user's request (2026-09-13) the hosted
GitHub "Check" workflow is disabled: it failed on every push and only
produced mail; verification stays local (`npm run check`, suites, proofs)
until the user asks for CI again. Next: rerun the 1M run and record it.
Completed plans remain **12 of 23**.

Previous completed iteration (2026-09-13): **plan 23 status correction**. Its
header still read "In progress … fault fixtures remain open" although every
task and criterion was ticked and the roadmap counted it complete; the header
now states Complete with the scale amendment.
...
in 1,570 s and then failed: the whole-file `PRAGMA integrity_check`
outlived the storage client's default 60 s reply deadline, and the same
scan ran at every startup through the light `diagnostics` read. Fixed under
ADR 0040 amendment 3: the client accepts a per-request deadline, the
explicit report is requested with 600 s and records the check's elapsed
time and file size, and the startup read verifies integrity only for files
up to 256 MiB (`unchecked` above, shown in onboarding). Three deadline
tests with a fake worker and a bounded-read test join the diagnostics
suite; the stress harness measures both reads. The 1M run is relaunched
and its result is recorded next. Completed plans remain **12 of 23**.

Previous completed iteration (2026-09-13): **handoff correction** (plan 06).
Plan 06 had every checkbox ticked while its remaining-scope notes still
named two real gates; the file now carries them as explicit open tasks
(checkpoint batching at product scale; live-provider qualification, a user
gate), so the open-item counts match the evidence. The 1M-message WebKit
stress run seeded 100,000 conversations in 1,570 s and is exporting; its
result is recorded next. Completed plans remain **12 of 23**.

Previous completed iteration (2026-09-12): **storage stress harness** (plan
24). `npm run test:app:stress:browser` seeds 100k conversations / 1M
messages through the production worker in bounded commits and measures
integrity, library paging, cold reopen, a concurrent tab, a portable
export streamed to disk in bounded steps and a restore validation streamed
back from disk; the page never holds the container. The smoke run (300
conversations) passes in both engines; the full 1M-message WebKit run was
launched and its result is recorded in
[storage-stress.md](../validation/storage-stress.md) when it completes.
Attachments at 10–50 GB exceed this machine's browser quota and are
recorded as not covered. Completed plans remain **12 of 23**.

Previous completed iteration (2026-09-12): **traceability checklist** (plan
24). [traceability.md](../validation/traceability.md) maps all nineteen §115
criteria, the §116 product-core requirements with semantic indexing
disabled, and the twelve §117 QuixiEmbed criteria to their plans, retained
scenarios, native host checks, benchmark reports and known limitations
(hosting, installers, Safari and Linux, real provider and export captures,
other GPU families, the relevance study). Plan 24's first task is ticked.
Completed plans remain **12 of 23**.

Previous completed iteration (2026-09-12): **portability search filter**
(plans 07/10) and **plan 09 marked complete**. The bulk analysis now
publishes each page's statuses to the storage owner (a derived local table
keyed by the target set; never exported), search applies the `portability`
filter in SQL before ranking, and the search form offers the filter with a
coverage note. Evidence: the search package's filter test (44 tests) and the
shared-app proof (101 checks per engine: exact matching hits for
the provider-dependent filter, none for blocked, the coverage note). Plan
07's dependent-gates task now lists only the OCR source refusal; plan 09
had every item ticked after the cross-host restore and is marked complete.
Completed plans: **12 of 23**.

Previous completed iteration (2026-09-12): **cross-host restore** (plan 09).
A new native proof (`npm run test:app:cross-host:native`) exports a
portable archive from the web host in Playwright Chromium, bundles the bytes
into a Tauri proof page built with the production desktop configuration,
restores them through the production Storage Worker inside the macOS
WebView into a validated candidate, reads the candidate back through the
retained-archive reader and hashes its blob files from OPFS, and requires
every collection's record digest, the record and sync-operation counts and
the blob hashes to equal the web export's. It passes
([cross-host-restore.md](../validation/cross-host-restore.md)); plan 09's
cross-host criterion is ticked. Completed plans remain **11 of 23**.

Previous completed iteration (2026-09-12): **plan 12 complete: reviewed
migration** (ADR 0042). The bulk analysis now carries per-target outcomes
and each conversation's primary; choosing a target marks conversations
eligible, a review lists what each would carry, and confirming commits per
conversation, against its revision, one `SetRoutingProfile` naming the
target and one `Migration` event; blocked, unchanged and failed
conversations are reported per conversation, Stop halts between them, and
nothing but routing changes. Evidence: 4 controller tests and the
shared-app proof (100 checks per engine: every eligible
conversation routed to GPT-4.1 mini after a review, Keep the current
routing leaving thread states byte-equal, one event and one primary change
per conversation, messages untouched, a migrated conversation opening on
the target with its event line)
([bulk-portability.md](../validation/bulk-portability.md)). Every plan 12
task and acceptance criterion is ticked. Completed plans: **11 of 23**.

Previous completed iteration (2026-09-12): **critique** (plan 12, ADR 0042).
"Critique this answer" on any sealed assistant answer starts one ordinary
attempt by the composer's selected model whose request carries the branch
through the reviewed answer plus a fixed review instruction; its output is
a sibling branch of the reviewed answer's parent, a `Critique` event
committed with the generation names the reviewed message and generation
and the critic, the critique's article carries a badge naming what it
reviewed, and the reviewed answer is never changed. Evidence: the
shared-app proof (99 checks per engine) checks the parent, the
event, the request's last turns, the badge and the byte-equal reviewed
message and parts ([compare.md](../validation/compare.md)). Plan 12's
critique task is ticked; the reviewed migration remains. Completed plans
remain **10 of 23**.

Previous completed iteration (2026-09-12): **compare mode** (plan 12, ADR
0042). One user turn goes to two to four models at once: the workflow
prepares and checks every candidate's request first, commits the turn once
with a `Compare` event naming each candidate's pre-allocated generation and
output, and runs one coordinated attempt per candidate concurrently without
moving the selection; Stop cancels them all. The conversation shows a
Compared answers section with each candidate's live status, tokens and
cost, Select this answer, and a Selected marker; the branch choices show
the same siblings. Evidence: the shared-app proof (98 checks per
engine) sends a turn to Claude Haiku 4.5 and GPT-4.1 mini, checks the two
generations, the event, the branches, selection, persistence across a
reload and continuation from the chosen answer, then fails the Anthropic
candidate mid-stream while the OpenAI one completes
([compare.md](../validation/compare.md)). Plan 12's compare tasks and two
acceptance criteria are ticked; critique and the reviewed migration remain.
Completed plans remain **10 of 23**.

Previous completed iteration (2026-09-12): **plan 12 opened: design and
bulk portability analysis** ([ADR 0042](../decisions/0042-compare-critique-bulk-migration.md)).
The ADR fixes compare as N independent attempts from one parent recorded
by a `Compare` event, critique as a Generation whose parameters and event
name the reviewed answer, and bulk migration as a paginated read-only
analysis followed by reviewed per-conversation routing changes. The
analysis is implemented: the chat workflow gained a per-conversation
portability assessment over its existing request path, and a Portability
section walks the library in 32-conversation pages against every
configured target, showing counts per outcome, per-conversation reasons,
an outcome filter, pages, Retry for failures and a way to open each
conversation. Evidence: 3 controller tests and the shared-app proof
(96 checks per engine: totals equal the library, counts sum with
zero failed, the seeded portability thread listed with its reasons)
([bulk-portability.md](../validation/bulk-portability.md)). Plan 12's
analysis task and its pagination criterion are ticked. Completed plans
remain **10 of 23**.

Previous completed iteration (2026-09-12): **plan 23 complete: repair fault
fixtures and the recovery foundation**. The diagnostics report now
classifies an unusable semantic namespace as rebuildable on its own check
(it was reported as "not enabled"); a `semantic-failure` fixture proves
Delete semantic index recreates the namespace with canonical counts
unchanged; the semantic proof verifies Rebuild semantic index against
canonical record and sync-operation counts and the message list; and the
[recovery-foundation table](../validation/diagnostics.md) names, per failure
class, what remains accessible, the recovery path and the proof. On the
way the Diagnostics panel gained an honest action state: semantic actions
run under the panel's busy state with the notice cleared, so a second
action can no longer be mistaken for the first. Evidence: 20 storage
diagnostics tests, the storage-health proof (19 checks per engine) and the
semantic proof (14 checks per engine). Every plan 23 task and acceptance
criterion is ticked. Completed plans: **10 of 23**.

Previous completed iteration (2026-09-12): **reviewed cleanup** (plan 23,
...
Next concrete slice: **record the 1M-message WebKit stress result** (check
the detached run's log, retain its report, fill the table in
storage-stress.md), then plan 24's PDF page-count cases (task 6) on the
existing extraction harnesses. The remaining open items of plans 01, 04,
05, 06, 07, 10, 11, 13, 19 and 21 are user, hardware or provider gates
listed in their files; do not contact real providers or spend on requests.
