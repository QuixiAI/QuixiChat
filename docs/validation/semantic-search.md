...
npm run test:app:semantic:browser     # shared application, Chromium + WebKit, real model (13 checks per engine)
...
| inference self-test (plan 23, 2026-09-12) | right after enrolment the Storage health "Run inference self-test" re-reads and re-hashes the model from its OPFS copy, reproduces the frozen token ids and reference vectors on the scalar and SIMD backends (max |Δ| 1.1e-7), runs the live WebGPU route on WebKit against the FP32 bound and reports Chromium headless's missing adapter as unsupported; `npm run test:app:semantic:browser`, 13 checks per engine (the saved diagnostics file carries the five inference checks), details in [diagnostics.md](diagnostics.md) |
