...
npm run test:app:semantic:browser     # shared application, Chromium + WebKit, real model (14 checks per engine)
...
| semantic rebuild verification (plan 23, 2026-09-12) | after the restart check, Rebuild semantic index starts a new generation, re-embeds all four chunks and answers the same semantic query while canonical record and sync-operation counts and the message list stay equal; `npm run test:app:semantic:browser`, 14 checks per engine |
| inference self-test (plan 23, 2026-09-12) | right after enrolment the Storage health "Run inference self-test" re-reads and re-hashes the model from its OPFS copy, reproduces the frozen token ids and reference vectors on the scalar and SIMD backends (max |Δ| 1.1e-7), runs the live WebGPU route on WebKit against the FP32 bound and reports Chromium headless's missing adapter as unsupported; `npm run test:app:semantic:browser`, 14 checks per engine (the saved diagnostics file carries the five inference checks), details in [diagnostics.md](diagnostics.md) |
| device-loss fallback in the application (plan 21, 2026-09-12) | the harness injects `gpu-device-loss` through the embedding service's diagnostics hook after enrolment; WebKit (WebGPU · FP32) embeds the next query and the resumed chunk through the CPU fallback and the panel reports "WASM SIMD · CPU"; Chromium's CPU route refuses the injection and keeps its backend; `npm run test:app:semantic:browser`, 11 checks per engine |
| semantic counters (ADR 0038 fix 8, 2026-09-12) | heads carry `vectors`/`failed`; `assertCountersExact` compares them with the exact per-chunk facts at the end of seven semantic scenarios (claims, rejections, leases and restarts, rebuild relinking and edits, deletion and damage, document pages, the projection lifecycle); the legacy-shape upgrade test drops all four head columns and both new indexes and checks the backfilled `indexedChunks`; Node 30k claim→publish round 183 → 70 ms |
| search profile (ADR 0038 fix 7, 2026-09-12) | obsolete scan armed only by replaced builds/heads or unpublished runs; candidate join forced knn → links → chunks (epoch, chunk id) → heads; per-head chunk counts for `indexedChunks`; the upgrade test stages the first build's shape (no flag, no count) and checks the backfilled count; Node 30k semantic query 294 → 16.5 ms |
