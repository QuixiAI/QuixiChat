...
| int8 projection (ADR 0036, 2026-09-12) | default threshold null keeps exact retrieval (`projection.threshold === null`, shown as "coarse stage off by measurement" in the browser proof); with `semanticCoarseThreshold: 3`, five published vectors are projected at once (`projection.complete`, `coarseRetrieval`), the coarse→rerank ranking equals the exact ranking of a second repository kept on the float path, quantization clamps to ±127; deleting ledger row 2 and both projection tables simulates a v1 namespace: reopen keeps the 5 float vectors, serves exact retrieval, and two-row maintenance backfills the projection; a foreign `representation` row is discarded at open and rebuilt; semantic deletion empties the projection while the lexical hit remains |
