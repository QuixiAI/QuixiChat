import type {HostClient,HostFile,StorageClient,ImportRun,ImportWorkGroup,EntityPage} from '@quixi/core/contracts';
...
export function createImportController(props:ImportPanelProps){
 let state:ImportSnapshot=Object.freeze({busy:false,discarding:false,choosing:false,error:null,notice:null,selectedFile:null,progress:null,runs:[],runsCursor:null,selectedRun:null,groups:[],groupsCursor:null,warnings:[],warningsCursor:null,warningGroup:null,availableBytes:null,report:null,reportBusy:false});
 ...
 const controller={
  ...
  async dispose(){if(disposed)return;cancelled=true;disposed=true;unsubscribe?.();unsubscribe=null;listeners.clear();await active?.catch(()=>{});await Promise.allSettled([releaseSelected(),releaseReport()]);},
 };
 return controller;
}
