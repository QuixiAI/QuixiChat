...
let armStage=false,stageHeld=false,releaseStage:(()=>void)|null=null;
const request=storage.request.bind(storage);
storage.request=async<K extends keyof StorageOperations>(requestId:string,operation:K,args:StorageOperations[K]['args']):Promise<StorageOperations[K]['result']>=>{const result=await request(requestId,operation,args);if(operation==='stageImportRecords'&&armStage){armStage=false;stageHeld=true;await new Promise<void>(resolve=>{releaseStage=resolve;});stageHeld=false;}return result;};
