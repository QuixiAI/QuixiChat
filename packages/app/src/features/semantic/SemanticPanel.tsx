  ...
  return <section aria-label="Semantic search" ref={focus.rootRef} onFocusCapture={focus.onFocusCapture}>
    ...
    {(!status || status.state === "disabled") ? <>
      ...
      <dl className="semantic-status">
        ...
        <dt>Candidate index</dt><dd data-testid="semantic-projection">{status.projection.projected.toLocaleString()} / {status.vectors.toLocaleString()} int8 · {status.projection.coarseRetrieval ? "coarse retrieval + exact rerank" : status.projection.threshold === null ? "exact retrieval (coarse stage off by measurement)" : status.vectors < status.projection.threshold ? `exact retrieval below ${status.projection.threshold.toLocaleString()} vectors` : "rebuilding, exact retrieval meanwhile"}</dd>
