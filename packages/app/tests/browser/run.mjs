...
try {
  ...
  for (const [name, engine] of selectedEngines) {
    ...
    try {
      ...
      // Reviewed migration (ADR 0042): route every eligible conversation to GPT-4.1 mini after a review; nothing but routing changes.
      const migrationSection = bulk.getByTestId("bulk-migration");
      const targetSelect = migrationSection.getByLabel("Migration target", { exact: true });
      const targetLabel = (await targetSelect.locator("option").allTextContents()).find((label) => /GPT-4\.1 mini$/.test(label));
      await targetSelect.selectOption({ label: targetLabel });
      const targetKey = await targetSelect.inputValue();
      const [targetConnection, targetModel] = targetKey.split("|");
      const eligibleButton = migrationSection.getByRole("button", { name: /^Select all eligible \(\d+\)$/ });
      const eligibleCount = Number((await eligibleButton.textContent()).match(/\((\d+)\)/)[1]);
      expect(eligibleCount).toBeGreaterThanOrEqual(1);
      await eligibleButton.click();
      await expect(bulk.getByTestId("migration-scope")).toContainText(`${eligibleCount} of ${eligibleCount} eligible conversations selected for ${targetLabel}`);
      const statesBeforeMigration = (await records(page, "threadStates")).items;
      const messagesBeforeMigration = (await records(page, "messages")).items.length;
      const migrationEventsBefore = (await records(page, "events")).items.filter((value) => value.type === "Migration").length;
      await migrationSection.getByRole("button", { name: "Review migration…", exact: true }).click();
      const migrationReview = bulk.getByTestId("migration-review");
      await expect(migrationReview).toContainText(`Route ${eligibleCount} conversation`);
      expect(await migrationReview.locator("ol > li").count()).toBe(eligibleCount);
      await migrationReview.getByRole("button", { name: "Keep the current routing", exact: true }).click();
      await expect(bulk.getByTestId("migration-review")).toHaveCount(0);
      expect((await records(page, "threadStates")).items).toEqual(statesBeforeMigration);
      await migrationSection.getByRole("button", { name: "Review migration…", exact: true }).click();
      await migrationReview.getByRole("button", { name: new RegExp(`^Migrate ${eligibleCount} conversation`) }).click();
      await expect(bulk.getByTestId("migration-result")).toContainText("Migration finished.", { timeout: 60_000 });
      await expect(bulk.getByTestId("migration-result")).toContainText(`${eligibleCount} migrated, 0 blocked, 0 unchanged, 0 failed`);
      const migratedTitles = await bulk.getByTestId("migration-result").locator("li[data-outcome='migrated']").evaluateAll((nodes) => nodes.map((node) => node.textContent.split(": ")[0]));
      const statesAfterMigration = (await records(page, "threadStates")).items;
      const migratedStates = statesAfterMigration.filter((value) => value.routingProfile?.primary?.provider === targetConnection && value.routingProfile?.primary?.model === targetModel && !statesBeforeMigration.some((before) => before.threadId === value.threadId && before.routingProfile?.primary?.provider === targetConnection && before.routingProfile?.primary?.model === targetModel));
      expect(migratedStates).toHaveLength(eligibleCount);
      const migrationEvents = (await records(page, "events")).items.filter((value) => value.type === "Migration");
      expect(migrationEvents).toHaveLength(migrationEventsBefore + eligibleCount);
      for (const event of migrationEvents.slice(-eligibleCount)) { expect(event.details.to).toEqual({ provider: targetConnection, model: targetModel }); expect(event.details.source).toBe("bulk-portability"); expect(migratedStates.some((value) => value.threadId === event.threadId)).toBe(true); }
      expect((await records(page, "messages")).items).toHaveLength(messagesBeforeMigration);
      expect(await bulkRows.locator("> li").filter({ hasText: "Migrated" }).count()).toBe(eligibleCount);
      await expect(eligibleButton).toHaveText("Select all eligible (0)");
      // A migrated conversation opens with the target as its selected connection and its Migration event listed.
      await bulkRows.locator("> li").filter({ hasText: migratedTitles[0] }).first().getByRole("button", { name: "Open conversation", exact: true }).click();
      await expect(page.getByLabel("Provider", { exact: true })).toHaveValue(targetConnection);
      await expect(page.getByRole("list", { name: "Conversation events" })).toContainText(/Migrated .* → .*\(reviewed bulk migration\)/);
      // A second reviewed migration routes the same conversations back to Claude Haiku 4.5: reversible, and later scenarios keep their expected connection.
      await page.getByRole("button", { name: "Portability", exact: true }).click();
      const backLabel = (await targetSelect.locator("option").allTextContents()).find((label) => /Claude Haiku 4\.5$/.test(label));
      await targetSelect.selectOption({ label: backLabel });
      const backKey = await targetSelect.inputValue();
      for (const title of migratedTitles) await bulkRows.locator("> li").filter({ hasText: title }).first().getByRole("checkbox", { name: `Migrate to ${backLabel}` }).check();
      await expect(bulk.getByTestId("migration-scope")).toContainText(`${migratedTitles.length} of `);
      await migrationSection.getByRole("button", { name: "Review migration…", exact: true }).click();
      await migrationReview.getByRole("button", { name: new RegExp(`^Migrate ${migratedTitles.length} conversation`) }).click();
      await expect(bulk.getByTestId("migration-result")).toContainText(`${migratedTitles.length} migrated, 0 blocked, 0 unchanged, 0 failed`, { timeout: 60_000 });
      const [backConnection, backModel] = backKey.split("|");
      const restored = (await records(page, "threadStates")).items.filter((value) => migratedStates.some((state) => state.threadId === value.threadId));
      expect(restored.every((value) => value.routingProfile?.primary?.provider === backConnection && value.routingProfile?.primary?.model === backModel)).toBe(true);
      ...
      const portableTitles = Object.entries(rowOutcomes).filter(([, outcome]) => outcome === "fully_portable").map(([title]) => title);
      const filtered = await searchWithFilter("fully_portable", "comet");
      expect(filtered.length).toBeGreaterThan(0);
      expect(filtered.every((title) => portableTitles.includes(title))).toBe(true);
      expect(filtered.length).toBe(unfiltered.filter((title) => portableTitles.includes(title)).length);
      ...
      // The image conversation's text matches "picture"; its status is not fully portable, so that filter excludes a real match.
      const dependentTitles = Object.entries(rowOutcomes).filter(([, outcome]) => outcome === "provider_dependent").map(([title]) => title);
      const pictureUnfiltered = await searchWithFilter("", "picture");
      expect(pictureUnfiltered.length).toBeGreaterThan(0);
      expect(pictureUnfiltered.some((title) => !portableTitles.includes(title))).toBe(true);
      const pictureFullyPortable = await searchWithFilter("fully_portable", "picture");
      expect(pictureFullyPortable.length).toBe(pictureUnfiltered.filter((title) => portableTitles.includes(title)).length);
      expect(pictureFullyPortable.length).toBeLessThan(pictureUnfiltered.length);
      const dependentHits = await searchWithFilter("provider_dependent", "picture");
      expect(dependentHits.every((title) => dependentTitles.includes(title))).toBe(true);
      expect(dependentHits.length).toBe(pictureUnfiltered.filter((title) => dependentTitles.includes(title)).length);
      const blockedHits = await searchWithFilter("blocked", "picture");
      ...
      evidence.portabilityFilter = { assessed: portabilityStatuses.assessed, comet: { unfiltered: unfiltered.length, fullyPortable: filtered.length }, picture: { unfiltered: pictureUnfiltered.length, fullyPortable: pictureFullyPortable.length, providerDependent: dependentHits.length, blocked: blockedHits.length } };
      evidence.checks.push(`the search form's portability filter uses the ${portabilityStatuses.assessed} statuses the library analysis published to the storage owner and is applied in the worker's SQL: "comet" keeps all ${filtered.length} of its ${unfiltered.length} hits under the fully-portable filter, "picture" drops from ${pictureUnfiltered.length} to ${pictureFullyPortable.length} under it and keeps ${dependentHits.length} under provider-dependent, the blocked filter returns none, and the coverage note names the analysis`);
      await page.getByRole("button", { name: "Portability", exact: true }).click();
