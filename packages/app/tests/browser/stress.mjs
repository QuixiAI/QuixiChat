...
try {
  ...
  for (const [name, engine] of selectedEngines) {
    ...
    try {
      ...
      evidence.phases.integrity = { ms: Math.round(diagnostics.ms), integrityMs: diagnostics.integrityMs, integrity: diagnostics.integrity, automaticIntegrity: diagnostics.automaticIntegrity, lightMs: Math.round(diagnostics.lightMs), databaseBytes: diagnostics.databaseBytes, canonicalRecords: diagnostics.canonicalRecords, syncOperations: diagnostics.syncOperations, usage: diagnostics.usage, quota: diagnostics.quota };
      expect(diagnostics.integrity).toBe("ok");
      expect(["ok", "unchecked"]).toContain(diagnostics.automaticIntegrity);
      log(name, `integrity ${diagnostics.integrity} in ${(diagnostics.integrityMs / 1000).toFixed(1)} s (report ${(diagnostics.ms / 1000).toFixed(1)} s; startup read ${diagnostics.automaticIntegrity} in ${diagnostics.lightMs.toFixed(0)} ms); ${diagnostics.canonicalRecords.toLocaleString()} records; database ${(diagnostics.databaseBytes / 1048576).toFixed(0)} MB; ${diagnostics.usage === null ? "usage unreported" : `${(diagnostics.usage / 1048576).toFixed(0)} MB used`}`);
      evidence.checks.push(`${SIZES.threads.toLocaleString()} conversations and ${total.toLocaleString()} messages seeded through the production worker in ${batches.toLocaleString()} bounded commits (${evidence.phases.seed.messagesPerSecond} messages/s); the explicit report's integrity_check answers ok in ${(diagnostics.integrityMs / 1000).toFixed(1)} s over ${diagnostics.canonicalRecords.toLocaleString()} canonical records (${(diagnostics.databaseBytes / 1048576).toFixed(0)} MB file) and the startup read answers ${diagnostics.automaticIntegrity} in ${diagnostics.lightMs.toFixed(0)} ms`);
