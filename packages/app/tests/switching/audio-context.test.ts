...
const pdf = new Uint8Array([82, 73, 70, 70, 4, 0, 0, 0, 87, 65, 86, 69]);
function fixture(options: { missing?: boolean; mediaType?: string; size?: number; malformed?: boolean; excluded?: boolean; copies?: number } = {}) {
  ...
  const attachment: Attachment = { id: attachmentId, availability: options.missing ? 'missing' : 'available',
    filename: 'original.wav', mimeType: options.mediaType ?? 'audio/wav', sizeBytes: options.size ?? pdf.length,
    blobSha256: 'a'.repeat(64), rawObjectId: null };
  const part: ContentPart = { id: partId, messageId: parent.id, order: 0, kind: 'Audio', data: { attachmentId, description: 'original.wav' } };
...
test('audio inspection, counting, send preparation and regeneration rehydrate the same original bytes and filename', async () => {
  for (const action of ['inspect', 'count', 'send', 'regenerate'] as const) {
    const f = fixture({ mediaType: 'audio/x-wav' });
    ...
    assert.equal(input.messages[0]!.parts[0]!.kind, 'Audio');
    assert.deepEqual(input.attachments?.[f.attachmentId], { mediaType: 'audio/wav', filename: 'original.wav', bytes: pdf });
    assert.equal(f.released.length, 1);
  }
});

test('missing and unsupported historical audio remain Audio parts without invented text or byte reads', async () => {
  for (const options of [{ missing: true }, { mediaType: 'application/zip' }, { size: LIMITS.audioBytes + 1 }]) {
    const f = fixture(options), inspection = await f.controller.inspectSwitch(f.provider, 'model');
    assert.equal(f.inputs[0]!.messages[0]!.parts[0]!.kind, 'Audio');
    assert.equal(f.inputs[0]!.attachments?.[f.attachmentId]?.bytes.length, 0);
    assert.equal(f.inputs[0]!.attachments?.[f.attachmentId]?.mediaType, options.mediaType ?? 'audio/wav');
    assert.equal(inspection!.transformed.unavailableImages, 0);
    assert(!f.reads.includes('readBlobTransfer'));
  }
});

test('explicitly excluded Audio reads no attachment metadata or bytes', async () => {
...
test('draft audio kind and filename reach counting and compatibility inspection without publication', async () => {
  const draft: ComposerAttachment = { kind: 'Audio', transferId: id(), sha256: 'b'.repeat(64), byteLength: pdf.length,
    mediaType: 'audio/wav', filename: 'draft.wav', bytes: pdf };
  for (const action of ['inspect', 'count'] as const) {
    ...
    assert.deepEqual(input.attachments?.[part.data.attachmentId], { mediaType: 'audio/wav', bytes: pdf, filename: 'draft.wav' });
  }
});

test('MP3 aliases normalize for dispatch while an excluded audio occurrence stays unread', async () => {
  const f = fixture({ mediaType: 'audio/mp3' });
  await f.controller.inspectSwitch(f.provider, 'model');
  assert.equal(f.inputs[0]!.attachments?.[f.attachmentId]?.mediaType, 'audio/mpeg');
  assert.equal(f.inputs[0]!.messages[0]!.parts[0]!.kind, 'Audio');
});
