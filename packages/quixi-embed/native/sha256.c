/* Fixed SHA-256 digest for package integrity (FIPS 180-4 arithmetic). */
#include "sha256.h"
#include <string.h>
...
static void block(uint32_t state[8], const uint8_t bytes[64]) {
  uint32_t w[64];
...
void qx_sha256(const uint8_t *data, size_t length, uint8_t digest[32]) {
  ...
  for(size_t i=0;i<full;i++) block(state,data+i*64);
  uint8_t tail[128]={0};size_t rest=length%64;
  if(rest) memcpy(tail,data+full*64,rest);
  tail[rest]=0x80;size_t size=rest<56?64:128;
  uint64_t bits=(uint64_t)length*8;
  for(unsigned i=0;i<8;i++) tail[size-1-i]=(uint8_t)(bits>>(i*8));
  block(state,tail);if(size==128) block(state,tail+64);
  for(unsigned i=0;i<8;i++) for(unsigned j=0;j<4;j++) digest[i*4+j]=(uint8_t)(state[i]>>(24-j*8));
}
