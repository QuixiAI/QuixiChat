...
spec = importlib.util.spec_from_file_location('scheduler_ci', Path(__file__).resolve().parents[1] / 'native/scheduler_ci.py')
ci = importlib.util.module_from_spec(spec)
spec.loader.exec_module(ci)


class SchedulerCIReportTest(unittest.TestCase):
    def test_failure_replaces_stale_success_and_identifies_command(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / 'build').mkdir()
            path = root / 'build/scheduler-ci-report.json'
            ...
            def run(command, **kwargs):
                ...
                return subprocess.CompletedProcess(command, 23 if any('run.mjs' in value for value in command) else 0)

            with patch.object(ci, 'ROOT', root), patch.object(ci.subprocess, 'run', side_effect=run):
                with self.assertRaises(subprocess.CalledProcessError):
                    ci.main(skip_provision=True)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(ci, 'ROOT', root), patch.object(ci.subprocess, 'run', side_effect=FileNotFoundError('missing tool')):
                with self.assertRaises(FileNotFoundError):
                    ci.main(skip_provision=True)
            report = json.loads((root / 'build/scheduler-ci-report.json').read_text())
            self.assertFalse(report['passed'])
