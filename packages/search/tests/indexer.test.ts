...
function fakeStorage(texts: string[], leaseMs = 30) {
  ...
  const status = (): SemanticIndexStatus => ({ state: state.state, model: identity, generation: state.generation, indexedChunks: chunks.filter((c) => c.vector).length, pendingChunks: chunks.filter((c) => !c.vector).length, vectors: chunks.filter((c) => c.vector).length, vectorBytes: 0, projection: { representation: "sign-bit-v1", bytesPerVector: 48, candidates: 5000, projected: chunks.filter((c) => c.vector).length, complete: true, coarseRetrieval: false, threshold: 100_000, residentBytes: 0 } });
  return {
    state, chunks,
