...
/** `stale` (2026-09-12, ADR 0038): the dirty triggers also flag the heads a
 * canonical change makes invisible, so visibility is the column test
 * `h.stale=0` instead of five correlated scope lookups per head. The legacy
 * form (no flag) is kept so a namespace recorded under it is recognised and
 * upgraded in place. */
const trigger = (event: "INSERT" | "UPDATE" | "DELETE", stale = true) => {
  const changed =
    event === "UPDATE"
...
CREATE TRIGGER quixi_search_dirty_${event.toLowerCase()} AFTER ${event} ON quixi_records WHEN ${row}.collection IN('parts','messages','generations','threads','threadStates','tombstones','documents','attachments','provenance','importSources')${changed} BEGIN
 ...
 INSERT INTO quixi_search_scopes(scope,id,revision) VALUES(${scope},${id},(SELECT revision FROM quixi_search_meta)) ON CONFLICT(scope,id) DO UPDATE SET revision=excluded.revision;${stale ? staleMarks(scope, id, row) : ""}
...
/** One indexed UPDATE per scope kind; the scope expression is constant for
 * the row, so each seek uses the head index for that column. */
const staleMarks = (scope: string, id: string, row: string) => `
 UPDATE quixi_search_heads SET stale=1 WHERE stale=0 AND ${scope}='source' AND source_key=${id};
 UPDATE quixi_search_heads SET stale=1 WHERE stale=0 AND ${scope}='message' AND message_id=${id};
 UPDATE quixi_search_heads SET stale=1 WHERE stale=0 AND ${scope}='thread' AND thread_id=${id};
 UPDATE quixi_search_heads SET stale=1 WHERE stale=0 AND ${scope}='document' AND document_id=${id};
 UPDATE quixi_search_heads SET stale=1 WHERE stale=0 AND ${scope}='global';
 UPDATE quixi_search_heads SET stale=1 WHERE stale=0 AND ${row}.collection='parts' AND json_extract(${row}.payload,'$.kind')='Image' AND source_key='f:'||${row}.id;`;
...
export const SEARCH_HEAD_STALE_INDEXES = `
...
CREATE INDEX quixi_search_head_visible ON quixi_search_heads(epoch,stale);
`;
const schemaText = (stale: boolean) => `
...
${stale ? SEARCH_TRIGGERS : SEARCH_TRIGGERS_LEGACY}
`;
export const SEARCH_SCHEMA = schemaText(true);
export const SEARCH_SCHEMA_CHECKSUM = searchDigest(SEARCH_SCHEMA);
/** The previous build's derived schema (no `stale` flag); recognised at open and upgraded in place. */
export const SEARCH_SCHEMA_LEGACY_CHECKSUM = searchDigest(schemaText(false));
...
/** Visibility is the trigger-maintained flag (ADR 0038). */
export const VISIBLE_HEAD = "h.stale=0";
/** The scope-revision comparison the flag replaces; used once to compute
 * `stale` for heads recorded by the previous build. */
export const LEGACY_VISIBLE_HEAD = `h.source_revision=coalesce((SELECT revision FROM quixi_search_scopes WHERE scope='source' AND id=h.source_key),0)
