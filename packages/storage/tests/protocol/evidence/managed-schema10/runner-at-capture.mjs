...
try {
  ...
  for (const [name, engine] of engines) {
    ...
    try {
      ...
      expect(owner.integrity).toBe('ok'); expect(owner.schemaVersion).toBe(10);
      ...
      host.observations.invalidBusCallIds = await call(page, 'invalidBusWrites', 'v2', owner.ownerId, selection);
      ...
      expect(snapshot(reopened)).toEqual(snapshot(final)); expect(reopened.schemaVersion).toBe(10);
      await call(page, 'closeClient', 'reopened');
      host.checks.push('After modern owner exits, frozen schema8 waiter acquires the same lock and refuses schema9 before its queued mutation; modern reopen preserves six committed operations and integrity');
