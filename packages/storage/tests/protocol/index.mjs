...
function rawRequest(name, operation, args, version = 4) {
...
window.protocolProof = {
  ...
  rawRequest,
  rawWrite(name, title, version = 4) { return rawRequest(name, 'commit', batch(title), version); },
  ...
  hello(name, version = 4) { buses.get(name).channel.postMessage({ ...(version === null ? {} : { version }), type: 'hello' }); },
  invalidBusWrites(name, ownerId, selection) {
    ...
    channel.postMessage({ version: 4, type: 'hello' });
    return ids;
  },
