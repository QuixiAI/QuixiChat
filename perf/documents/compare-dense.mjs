...
if(names.length!==2 || names.some(name=>!/^dense-[a-z0-9-]+$/.test(name))) throw new Error('Pass baseline and cleanup dense-* report basenames');
...
if(JSON.stringify(differences)!=='["packages/documents/src/worker/index.ts"]') throw new Error('Unexpected source difference: '+JSON.stringify(differences));
...
await writeFile(resolve(import.meta.dirname,'results/dense-cleanup-comparison.json'),JSON.stringify(result,null,2)+'\n');
