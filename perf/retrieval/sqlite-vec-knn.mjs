...
const coarseRerank = time((q) => db.exec({ sql: "SELECT v.rowid,vec_distance_L2(v.embedding,?) AS d FROM f v WHERE v.rowid IN (SELECT rowid FROM q8 WHERE embedding MATCH vec_int8(?) AND k=500) ORDER BY d LIMIT 64", bind: [new Uint8Array(q.buffer), new Uint8Array(int8(q).buffer)], returnValue: "resultRows" }));
// Agreement between the coarse→rerank top-64 and the float top-64 on these pools.
let agree = 0; for (const q of queries) { const a = db.exec({ sql: "SELECT rowid FROM f WHERE embedding MATCH ? AND k=64", bind: [new Uint8Array(q.buffer)], returnValue: "resultRows" }).map((r) => r[0]); const b = new Set(db.exec({ sql: "SELECT v.rowid FROM f v WHERE v.rowid IN (SELECT rowid FROM q8 WHERE embedding MATCH vec_int8(?) AND k=500) ORDER BY vec_distance_L2(v.embedding,?) LIMIT 64", bind: [new Uint8Array(int8(q).buffer), new Uint8Array(q.buffer)], returnValue: "resultRows" }).map((r) => r[0])); agree += a.filter((id) => b.has(id)).length / a.length; }
