...
await build({
  configFile: false, root: import.meta.dirname, plugins: [pdfAssetNotices()],
  worker: { format: 'es' }, logLevel: 'warn',
  build: { outDir: resolve(output, 'dist'), emptyOutDir: true, assetsInlineLimit: 0 },
});
const production = JSON.parse(await readFile(resolve(root, 'apps/desktop/src-tauri/tauri.conf.json'), 'utf8'));
const proof = {
  ...production,
  productName: 'Quixi Regional App Proof', identifier: 'ai.quixi.chat.regional-app-proof',
